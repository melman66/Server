...
```explicit MessagesDB(const QString &host, const QString &username)``` - В него передается адрес сервера и имя клиента. В теле конструктора вызывается метод ```openDatabase```, возвращаемый резульятат которого присваивается атрибуту ```stateDB```.
